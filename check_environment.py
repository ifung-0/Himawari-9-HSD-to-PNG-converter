...
def run_checks() -> list[CheckResult]:
    results = [
        ...
        CheckResult("python version", True, sys.version.split()[0], critical=False),
...
def run_fix() -> int:
    if not REQUIREMENTS_FILE.exists():
        ...
        return 1
    command = pip_install_command(upgrade=True)
...
def main() -> int:
    ...
    if args.fix and has_critical_failures(results):
        print()
        fix_code = run_fix()
        if fix_code != 0:
            return fix_code
    ...
    if has_critical_failures(results):
        print()
        print("One or more critical checks failed. Run this same script with --fix, or install requirements with:")
        print(" ".join(f'"{part}"' if " " in part else part for part in pip_install_command(upgrade=True)))
        return 1
