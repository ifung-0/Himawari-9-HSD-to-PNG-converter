...
class InstallRequirementsTests(unittest.TestCase):
    ...
    def test_main_installs_overlay_data_after_packages(self, mock_overlay_install, mock_call):
        mock_overlay_install.return_value = check_environment.OverlayInstallResult(True, True, "ok")

        with mock.patch("install_requirements.sys.argv", ["install_requirements.py", "--upgrade"]):
    ...
    @mock.patch("install_requirements.check_environment.install_overlay_data")
    def test_main_can_skip_overlay_data(self, mock_overlay_install, mock_overlay_folder, _mock_call):
        with mock.patch("install_requirements.sys.argv", ["install_requirements.py", "--skip-overlay-data"]):
            result = installer.main()
    ...
    @mock.patch("install_requirements.subprocess.call", return_value=0)
    @mock.patch("install_requirements.check_environment.install_overlay_data")
    def test_main_returns_failure_when_overlay_install_fails(self, mock_overlay_install, _mock_call):
        mock_overlay_install.return_value = check_environment.OverlayInstallResult(False, False, "failed")

        with mock.patch("install_requirements.sys.argv", ["install_requirements.py"]):
            result = installer.main()
    ...
    @mock.patch("install_requirements.subprocess.call", return_value=0)
    @mock.patch("install_requirements.check_environment.install_overlay_data")
    def test_main_can_force_overlay_reinstall(self, mock_overlay_install, _mock_call):
        mock_overlay_install.return_value = check_environment.OverlayInstallResult(True, True, "ok")

        with mock.patch("install_requirements.sys.argv", ["install_requirements.py", "--force-overlay-data"]):
            result = installer.main()
